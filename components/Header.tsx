    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-8 text-sm font-medium tracking-wide uppercase">
          ...
          <button onClick={() => { if(currentPage!=='home') onNavigate('home'); setTimeout(() => document.getElementById('portfolio')?.scrollIntoView({ behavior: 'smooth' }), 100); }} className="hover:text-accent transition-colors">Cases</button>
          <button onClick={() => { if(currentPage!=='home') onNavigate('home'); setTimeout(() => document.getElementById('about')?.scrollIntoView({ behavior: 'smooth' }), 100); }} className="hover:text-accent transition-colors">DNA Story</button>
        ...
        <button onClick={() => onNavigate('contact')} className="md:hidden text-accent">
